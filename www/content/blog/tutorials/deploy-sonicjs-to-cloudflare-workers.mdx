...
You should see `users`, `collections`, `content`, `content_versions`, `media`, `api_tokens`, and the plugin tables. For a deeper look at the schema and how Drizzle generates migrations, read the [database documentation](/database).

## Step 3: Create the KV Namespace

KV holds SonicJS's edge cache — API responses, collection metadata, and verified JWTs. Create the namespace:
...
**`D1_ERROR: no such table: users`** — Migrations haven't run on the remote DB. Run `npm run db:migrate` (which is `wrangler d1 migrations apply DB`).
