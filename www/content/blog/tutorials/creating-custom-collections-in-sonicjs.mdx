...
<TLDRBox>
**TL;DR** — Collections are the heart of any SonicJS project: a single TypeScript file describes your content type, and SonicJS turns it into a typed database table, an admin UI, and a full REST API. Define a `CollectionConfig`, register it with `registerCollections([...])`, and let auto-sync handle the rest.
...
1. Creates or updates the underlying [database](/database) table
...
Two flavors are supported in the same project — see the [collections reference](/collections) for the full breakdown:

- **Code-based collections** — defined in TypeScript, auto-synced on startup, version-controlled. Best for production.
- **UI-created collections** — built in the admin UI, stored in the database. Best for prototyping.
